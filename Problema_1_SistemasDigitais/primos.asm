	...
	.text
		...
		li $s4, 10						#QUANTIDADE DE NUMEROS PRIMOS(entrada)
		...
		la $t2, vetor					#pega o valor do gp para armazenar os numeros primos
		...
		VERIFY:
			...
			bne $s3, $t1, ENDVERIFY			#se nao houver apenas 2 divisoes corretas(nao e primo)

			addi $s4, $s4, -1			#se for, contabiliza mais um numero primo na quantidade informada

			sw $s0, 0($t2)				#armazena na memoria

			addi $t2, $t2, 4			#incrementa a posicao da memoria para um possivel numero primo

		

		ENDVERIFY:

			beq $s4, $zero, END		#se for o ultimo valor a ser verificado, encerra o programa
